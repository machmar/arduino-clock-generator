...
int menuEncoderVar9 = 0;
int menuEncoderVar10 = 0;
int decimalPoint = 0;
...
if (swingVar == 0) {
    //interval1 = reverseDelay * swing;
    Serial.print(interval1);
    Serial.print(" = ");
    Serial.print(reverseDelay);
    Serial.print(" * ");
    Serial.println(swing);
    Serial.println(reverseDelay * swing);
    interval0 = reverseDelay * swing;
    interval1 = interval0;
   }
   
   if (swingVar == 1) {
    //interval1 = reverseDelay * (100 - swing);
    Serial.print(interval1);
    Serial.print(" = ");
    Serial.print(reverseDelay);
    Serial.print(" * (100 - ");
    Serial.print(swing);
    Serial.println(")");
    Serial.println(reverseDelay * (100 - swing));
    interval0 = reverseDelay * (100 - swing);
  ...
  if (currentMillis - previousMillis1 >= interval1 / 100) {
...
void setup() {
  ...
  startingValue = EEPROM.read(101);
  bpmEncoderRotation = startingValue;
startupLoad = EEPROM.read(100);
if (startupLoad == 0) {
  swing = EEPROM.read(0);
...
void loop() {
  ...
  bpm = bpmEncoderRotation / bpmDivide * bpmMultiply; //this converts the raw encoder data to usable data, the /2 is there because when there was nothing my encoder
  ...
  if (menuVar2 == 2) { //--------------------MAIN-MENU--------------
    ...
    if (menuEncoderVar1 == 7) {
      sevseg.setChars("STVA");
      sevseg.refreshDisplay();
      if (normalClick == 1) {
        normalClick = 0;
        menuVar2 = 19;
    ...
    if (menuEncoderVar1 == 9) menuEncoderVar1 = 8;
  ...
  if (menuVar2 == 17) {   //-----------------------------STLO-SUBMENU-----------------------
    ...
    if (menuEncoderVar9 == 1) {
      if (startupLoad == 0) {
        ...
        if (normalClick == 1) {
          startupLoad = 0;
          EEPROM.write(100, startupLoad);
          normalClick = 0;
        }
      }
      }
      if (menuEncoderVar9 == 2) {
        if (startupLoad == 1) {
          sevseg.setChars("-OFF");
          sevseg.refreshDisplay();
        }
      ...
      if (menuEncoderVar9 == 3) menuEncoderVar9 = 2;
      if (menuEncoderVar9 == -1) menuEncoderVar9 = 0;
...
if (menuVar2 == 19) {   //-----------------------------STVA-SUBMENU-----------------------
    if (result == DIR_CW) {
     startingValue ++;
    } else if (result == DIR_CCW) {
      startingValue --;
    }
     sevseg.setNumber(startingValue);
        sevseg.refreshDisplay();
        if (normalClick == 1) {
          EEPROM.write(101, startingValue);
          normalClick = 0;
          menuVar2 == 18;
        }
      }
